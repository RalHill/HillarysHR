...
export default function HRFormulasCalculator() {
  ...
  // Validation and calculation helper
  ...
  const calcTimeToHire = useMemo(() => {
    ...
    if (accepted < applied) {
      return { error: 'Acceptance date must be after application date' }
    }
  ...
  const calcHumanCapitalROI = useMemo(() => {
    ...
    if (isNaN(rev) || isNaN(costs)) {
      return { error: 'Enter all values' }
    }
    if (costs === 0) {
      return { error: 'Compensation costs cannot be 0' }
    }
  ...
  const FormulaSection = React.memo(({ title, explanation, children, calcResult }: { title: string; explanation: string; children: React.ReactNode; calcResult: { error?: string; result?: number } }) => {
    ...
    return (
      <div className="tool-card-elevated" style={{ padding: '20px', marginBottom: '20px' }}>
        ...
        {!isError && calculation.result !== undefined && (
          <div style={{ marginTop: '12px' }}>
            ...
            <div className="tool-result-value" style={{ fontSize: '1.35rem' }}>
              {title.includes('Cost Per Hire')
                ? `$${calculation.result.toLocaleString('en-CA', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                : `${calculation.result.toFixed(2)}${title === 'Time to Hire' ? ' days' : '%'}`}
            </div>
      ...
      <div className="tool-page-inner">
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          <FormulaSection
            ...
            explanation="The percentage of work time lost due to absences. Lower is better."
            calcResult={calcAbsenteeism}
          >
          ...
          <FormulaSection
            ...
            explanation="The percentage of employees who left during the period. Helps track turnover trends."
            calcResult={calcAttrition}
          >
          ...
          <FormulaSection
            ...
            explanation="The percentage of employees retained from the start to end of the period. Higher is better."
            calcResult={calcRetention}
          >
          ...
          <FormulaSection
            ...
            explanation="The percentage of positions filled by internal promotions. Higher indicates strong development culture."
            calcResult={calcPromotion}
          >
          ...
          <FormulaSection
            title="Cost Per Hire"
            explanation="The average cost to recruit, interview, and onboard one new employee."
            calcResult={calcCostPerHire}
          >
          ...
          <FormulaSection
            title="Time to Hire"
            explanation="The number of calendar days from application to offer acceptance. Lower is more efficient."
            calcResult={calcTimeToHire}
          >
          ...
          <FormulaSection
            ...
            explanation="The percentage of job offers that are accepted. Indicates role and offer competitiveness."
            calcResult={calcOfferAcceptance}
          >
          ...
          <FormulaSection
            ...
            explanation="The return on investment from your total payroll and compensation spend. Helps assess workforce profitability."
            calcResult={calcHumanCapitalROI}
          >
          ...
          <FormulaSection
            ...
            explanation="The percentage of survey responses that were positive. Higher indicates better morale and engagement."
            calcResult={calcEngagement}
          >
